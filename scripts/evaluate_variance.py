"""
Evaluate models multiple times with different random samples to calculate variance/std/min/max.
"""
...
def evaluate_model(model, tokenizer, test_df, num_samples=250, seed=42):
    ...
    predictions = []
    labels = [1 if label == 'sarc' else 0 for label in sample_df['class'].tolist()]
...
def main():
    # Load test data
    test_df = pd.read_csv('data/splits/gen_test.csv')
    print(f"Total test samples available: {len(test_df)}")
    
    num_runs = 10
    num_samples_per_run = 500
    ...
    output_path = Path('results/variance_analysis.json')
